...
[[ "${RUN_ID}" == HGV1_SPEEDCAL_* ]] || {
  echo "invalid HINT-gold speed calibration run id" >&2
  exit 2
}
...
[[ ! -e "${RESULT}" ]] || {
  echo "speed calibration already finished" >&2
  exit 2
}
...
set +e
APOLLO_EXTRA_PYTHONPATH="${APOLLO_EXTRA}" "${CAGE_BUNDLE_ROOT}/scripts/apollo_host_exec.sh" \
  python3 "${REPO_ROOT}/scripts/d0/hint_gold/carla_speed_band_calibration.py" \
  --trace "${TRACE}" --summary "${SUMMARY}" >"${LOG_ROOT}/calibration.log" 2>&1
runtime_exit=$?
set -e
cleanup
ended_ns="$(date +%s%N)"
powered="$(${CAGE_RUNTIME_ROOT}/envs/cage-ad-py310/bin/python -c \
  'import sys; print((int(sys.argv[2])-int(sys.argv[1]))/1e9)' "${started_ns}" "${ended_ns}")"

${CAGE_RUNTIME_ROOT}/envs/cage-ad-py310/bin/python - \
  "${RUN_ID}" "${SUMMARY}" "${runtime_exit}" "${powered}" \
  "$(git -C "${REPO_ROOT}" rev-parse HEAD)" "${RESULT}" <<'PY'
import json
import os
import pathlib
import sys
...
result = {
    ...
    "result": (
        "PASS"
        if int(runtime_exit) == 0 and summary and summary.get("passed")
        else "FAIL"
    ),
...
print(json.dumps({"result": result["result"], "powered_on_seconds": result["powered_on_seconds"]}, sort_keys=True))
