...
: "${CAGE_BUNDLE_ROOT:?}"
: "${CAGE_RUNTIME_ROOT:?}"
[[ "${RUN_ID}" == CARLA_V16A_* ]] || { echo "invalid v16a run id" >&2; exit 2; }
...
[[ ! -e "${RESULT}" ]] || { echo "v16a physics audit already finished" >&2; exit 2; }
...
set +e
APOLLO_EXTRA_PYTHONPATH="${APOLLO_EXTRA}" "${CAGE_BUNDLE_ROOT}/scripts/apollo_host_exec.sh" python3 "${REPO_ROOT}/scripts/d0/repair/carla_full_throttle_physics_audit.py" --trace "${TRACE}" --summary "${SUMMARY}" >"${LOG_ROOT}/physics_audit.log" 2>&1
runtime_exit=$?
set -e
cleanup
ended_ns="$(date +%s%N)"
...
result = {
    ...
    "run_id": run_id,
    "result": "PASS" if int(runtime_exit) == 0 and summary and summary.get("passed") else "STOP",
...
print(json.dumps({"result": result["result"], "verdict": None if not summary else summary["verdict"]}, sort_keys=True))
