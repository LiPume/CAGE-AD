...
: "${CAGE_BUNDLE_ROOT:?}"
: "${CAGE_RUNTIME_ROOT:?}"
[[ "${RUN_ID}" == CARLA_V16_* ]] || { echo "invalid v16 run id" >&2; exit 2; }
...
[[ ! -e "${RESULT}" ]] || { echo "v16 calibration already finished" >&2; exit 2; }
install -d -m 0700 "${RUN_STATE}" "${RUN_DATA}" "${LOG_ROOT}"
cleanup() { "${CAGE_BUNDLE_ROOT}/scripts/manage_carla_server.sh" stop >>"${LOG_ROOT}/cleanup.log" 2>&1 || true; }
...
set +e
APOLLO_EXTRA_PYTHONPATH="${APOLLO_EXTRA}" "${CAGE_BUNDLE_ROOT}/scripts/apollo_host_exec.sh" python3 "${REPO_ROOT}/scripts/d0/repair/carla_multi_speed_calibration.py" --trace "${TRACE}" --summary "${SUMMARY}" >"${LOG_ROOT}/calibration.log" 2>&1
runtime_exit=$?
set -e
cleanup
ended_ns="$(date +%s%N)"
...
result = {"schema_version": 1, "label": "CONTROL_LOOP_DIAGNOSTIC_NOT_DATASET", "run_id": run_id, "result": "PASS" if int(runtime_exit) == 0 and summary and summary.get("passed") else "FAIL", "runtime_exit": int(runtime_exit), "powered_on_seconds": float(powered), "source_commit": commit, "summary": summary}
path = pathlib.Path(output); temporary = path.with_suffix(path.suffix + f".tmp.{os.getpid()}")
