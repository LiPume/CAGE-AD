#!/usr/bin/env python3
"""Measure CARLA throttle response from three frozen starting speeds."""
...
def spawn(world: carla.World) -> carla.Vehicle:
    ...
    blueprint.set_attribute("role_name", "v16_calibration_vehicle")
...
def main() -> None:
    ...
    if not world.get_map().name.endswith("/Town01"):
        raise RuntimeError("v16 calibration requires preloaded Town01")
    if world.get_actors().filter("vehicle.*"):
        raise RuntimeError("v16 calibration requires zero existing vehicles")
            ...
            for throttle in LEVELS:
                for repeat in range(REPEATS):
                    ...
                    samples.append({
                        ...
                        "speed_slope_mps2": speed_slope(evaluation) if evaluation else None,
    ...
    for target_speed in TARGET_SPEEDS:
        for throttle in LEVELS:
            ...
            slopes = [sample["speed_slope_mps2"] for sample in group if sample["speed_slope_mps2"] is not None]
            profiles.append({
                "target_start_speed_mps": target_speed,
                "throttle": throttle,
                "repeat_count": len(group),
                "median_speed_slope_mps2": median(slopes) if len(slopes) == REPEATS else None,
                "repeat_speed_slope_range_mps2": max(slopes) - min(slopes) if len(slopes) == REPEATS else None,
                "samples": group,
            })
    checks = {
        "all_45_samples_present": len(samples) == len(TARGET_SPEEDS) * len(LEVELS) * REPEATS,
        "all_preconditions_reached": all(sample["precondition_reached"] for sample in samples),
        "all_start_speeds_in_range": all(
            sample["target_start_speed_mps"] <= sample["start_speed_mps"]
            <= sample["target_start_speed_mps"] + 0.15 for sample in samples
        ),
        ...
        "repeat_ranges_at_most_0_20": all(
            ...
            and profile["repeat_speed_slope_range_mps2"] <= 0.20 for profile in profiles
        ),
    }
    for target_speed in TARGET_SPEEDS:
        values = [
            profile["median_speed_slope_mps2"] for profile in profiles
            if profile["target_start_speed_mps"] == target_speed
            and profile["median_speed_slope_mps2"] is not None
        ]
        checks[f"speed_{target_speed}_monotonic"] = len(values) == len(LEVELS) and all(
            later + 0.05 >= earlier for earlier, later in zip(values, values[1:])
        )
        checks[f"speed_{target_speed}_spans_low"] = bool(values) and min(values) <= 0.10
        checks[f"speed_{target_speed}_spans_high"] = bool(values) and max(values) >= 0.35
    summary = {
        "schema_version": 1,
