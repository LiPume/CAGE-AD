...
后续数据生成已经改用 `benchmarks/apollo_d0/protocol_v1/` 与
`docs/dataset/CAGE_AD_D0_GENERATION_PROTOCOL.md`。该协议把 12 条改为逐项 calibration recipes，并取消“全笛卡尔积后直接扩成 36 条”的做法。本说明书第 2–6 节以下内容保留为 v3 历史事实，不能用于生成 protocol v1 新数据。
